...
void handle_int(int sig) {
    ...
    if (bus) {
        ...
        bus->shutdown = 1;
        if (bus->driver_pid) {
            if (kill(bus->driver_pid, SIGUSR2) == -1) {
                perror("kill driver SIGUSR2");
            }
        }
    }
    char b[64];
    ts(b, sizeof(b));
    char ln[128];
    snprintf(ln, sizeof(ln), "[%s] [DYSPOZYTOR] SIGINT\n", b);
    log_write(ln);
    _exit(0);
}
...
int main() {
    ...
    if (bus->driver_pid) {
        if (kill(bus->driver_pid, SIGUSR1) == -1) {
            perror("kill driver SIGUSR1");
        }
        char b[64];
        ts(b, sizeof(b));
        char ln[128];
    ...
    ts(b2, sizeof(b2));
    char ln2[128];
    snprintf(ln2, sizeof(ln2), "[%s] [DYSPOZYTOR] Koniec pracy\n", b2);
    log_write(ln2);
