...
int main() {
    ...
    if (msgsnd(msgid, &m, sizeof(m) - sizeof(long), 0) == -1) perror("msgsnd register");
    if (!vip) {
        int got = 0;
        for (;;) {
            ssize_t rr = msgrcv(msgid, &m, sizeof(m) - sizeof(long), getpid(), IPC_NOWAIT);
            if (rr >= 0) { got = 1; break; }
            if (errno != ENOMSG) { perror("msgrcv ticket"); break; }
            sem_lock();
            int shutdown = bus->shutdown;
            int blocked = bus->station_blocked;
            sem_unlock();
            if (shutdown || blocked) break;
            struct timespec tsd; tsd.tv_sec = 0; tsd.tv_nsec = 10000000L; nanosleep(&tsd, NULL);
        }
        if (!got || !m.ticket_ok) {
